...
RSpec.describe Blacklight::FacetFieldPaginationComponent, type: :component do
  ...
  context 'when show_sort is false' do
    before { render_component(show_sort: false) }

    it 'renders the paging links only (no sort dropdown)' do
      expect(page).to have_css('.prev_next_links')
      expect(page).to have_no_css('.facet-sort-dropdown')
    end
  end
end
