...
class Openalex
  ...
  def retrieve_metadata_by_ids(ids:, type:)
    ...
    when 'PMID'
      id_path = "#{@base_prefix}works?filter=ids.pmid:#{query_ids}" \
                '&select=id,ids,title,doi,publication_year,primary_location,type,authorships'
    when 'OpenAlex'
      id_path = "#{@base_prefix}works?filter=openalex:#{query_ids}" \
                '&select=id,ids,title,doi,publication_year,primary_location,type,authorships'
    else
      dois = ids.map { |id| "https://doi.org/#{id}" }.join('|')
      id_path = "#{@base_prefix}works?filter=doi:#{dois}" \
                '&select=id,ids,title,doi,publication_year,primary_location,type,authorships'
    end
